...
```sh
make build        # build binary to ./zsh_poundai
make test         # go test ./...
make install      # install to ~/.local/bin (override with PREFIX=)
make uninstall    # remove from ~/.local/bin
```

The end-to-end test in `cmd/zsh_poundai/main_test.go` builds the binary and runs it against a mock HTTP server. It is skipped with `-short`.
...
Config lives at `$XDG_CONFIG_HOME/zsh_poundai.yaml`. The loader uses `yaml.KnownFields(true)` — unknown keys cause hard errors. See `config.example.yaml` for the full schema.
...
- Set version at build time: `go build -ldflags "-X github.com/elee1766/zsh_poundai/pkg/cli.Version=v1.0.0" ./cmd/zsh_poundai`.
